...
def run_game():
    ...
    while True:
        if falling_fig == None:
            # Åñëè íåò ïàäàþùèõ ôèãóð - ãåíåðèðóåì íîâóþ
            falling_fig = next_fig
            next_fig = get_new_fig()
            LAST_FALL = time.time()

            if not check_pos(cup, falling_fig):
                return  # Åñëè íà èãðîâîì ïîëå íåò ñâîáîäíîãî ìåñòà - èãðà çàêîí÷åíà
        quit_game()
        for event in pygame.event.get():
            if event.type == KEYUP:
                """Ñîáûòèÿ ïðè îòïóñêàíèè êíîïêè"""
                ...
                """Îáðàáîòêà ñîáûòèé íàæàòèÿ íà êíîïêó"""
                ...
                # Ïîâîðà÷èâàåì ôèãóðó, åñëè åñòü ìåñòî
                    ...
                    """Ìãíîâåííîå ïàäåíèå ôèãóðû âíèç"""
        ...
        """Óïðàâëåíèå ïàäåíèåì ôèãóðû ïðè óäåðæèâàíèè êëàâèø"""
        ...
        """Îòðèñîâêà îêíà èãðû ñî âñåìè íàäïèñÿìè"""
...
def draw_info(points, level):
    ...
    esc_surf = BASIC_FONT.render(f'Пауза: пробел', True, INFO_COLOR)
...
def draw_block(block_x, block_y, color, pixel_x=None, pixel_y=None):
    ...
    if color == EMPTY:
        return
    if pixel_x == None and pixel_y is None:
